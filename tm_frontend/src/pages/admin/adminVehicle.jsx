        ...
        <div>
            ...
            <div className="relative flex bg-white rounded-2xl border border-[#0D4F46]/15 shadow-sm overflow-hidden mb-8">
                <div className="flex-1 p-8">
                    <p className="uppercase tracking-[0.25em] text-xs text-[#1A7A6E] font-semibold mb-2"> Admin · Fleet Registry </p>
                    <h1 className="font-serif text-4xl text-[#0D2A24]"> Vehicle Management </h1>
                    <p className="text-[#4B5F59] mt-2"> Manage all tour vehicles </p>
                </div>
                ...
                <div className="relative flex flex-col items-center justify-center px-10 border-l-2 border-dashed border-[#0D4F46]/25 bg-[#0D4F46] min-w-[220px]">
                    ...
                    <p className="text-[10px] tracking-[0.25em] uppercase text-[#C9A84C]"> Fleet </p>
                    <p className="font-serif text-2xl text-white mt-1"> {vehicles.length} Vehicles </p>
            ...
            <div className="bg-white rounded-2xl border border-[#0D4F46]/15 overflow-hidden">
                <div className="px-8 py-6 border-b border-[#0D4F46]/15">
                    ...
                    <h2 className="font-serif text-2xl text-[#0D2A24] mt-1"> Fleet Vehicles </h2>
                </div>

                <table className="w-full">
                    <thead>
                        <tr className="bg-[#0D4F46] text-[#F5EDD6]">
                            <th className="px-6 py-3 text-left text-xs uppercase tracking-wide font-semibold">Vehicle</th>
                            <th className="px-6 py-3 text-left text-xs uppercase tracking-wide font-semibold">Type</th>
                            <th className="px-6 py-3 text-left text-xs uppercase tracking-wide font-semibold">Number</th>
                            <th className="px-6 py-3 text-left text-xs uppercase tracking-wide font-semibold">Seats</th>
                            <th className="px-6 py-3 text-left text-xs uppercase tracking-wide font-semibold">Driver</th>
                            <th className="px-6 py-3 text-left text-xs uppercase tracking-wide font-semibold">Price/Day</th>
                    ...
                    <tbody>
                        {vehicles.map((vehicle) => (
                            <tr key={vehicle.id} className="border-b border-[#0D4F46]/10 hover:bg-[#F5EDD6]/70">
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        <img src={vehicle.image} alt={vehicle.name} className="w-14 h-14 rounded-lg object-cover border border-[#0D4F46]/15" />
                                        <span className="font-serif text-[#0D2A24]"> {vehicle.name} </span>
                                    </div>
                                </td>
                                <td className="px-6 py-4 text-[#4B5F59]">{vehicle.type}</td>
                                <td className="px-6 py-4 text-[#4B5F59]"> {vehicle.number} </td>
                                <td className="px-6 py-4 text-[#4B5F59]"> {vehicle.seats} </td>
                                <td className="px-6 py-4 text-[#4B5F59]"> {vehicle.driver} </td>
                                <td className="px-6 py-4 font-serif text-[#C9A84C] font-semibold"> Rs. {vehicle.price.toLocaleString()} </td>
                                <td className="px-6 py-4">
                                    <span
